...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--channel", choices=CI_CHANNELS, required=True, help="CI prerelease channel to generate.")
...
def find_latest_stable_release(releases: list[dict[str, Any]]) -> StableRelease:
    # 从所有 release 中挑出最新稳定版，作为下一次预发布的基线。
...
def is_run_after_release(run: dict[str, Any], published_at: datetime) -> bool:
    created_at = run.get("created_at")
    return bool(created_at and parse_iso8601(created_at) > published_at)


def reserves_channel_number(run: dict[str, Any], channel: str, published_at: datetime) -> bool:
    """Return whether a workflow run reserves a number for the requested channel."""
    if not is_run_after_release(run, published_at):
        return False
...
def build_ci_version(
    ...
    # 只统计最新正式 release 发布之后的同通道 run，失败或取消的 run 也占号。
    runs_after_release = [
        run
        for run in runs
        if reserves_channel_number(run, channel, latest_release.published_at)
    ]
    ...
    return f"V{next_version[0]}.{next_version[1]}.{next_version[2]}-{channel}.{build_number}"
...
def load_releases_and_runs(args: argparse.Namespace) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    runs_url = (
        f"{args.github_api_url}/repos/{args.repo}/actions/workflows/{workflow_path}/runs"
    )
...
def main() -> int:
    # 版本计算失败时直接失败，避免把 SHA 等非公开版本号带入构建和发布。
    args = parse_args()
    try:
        releases, runs = load_releases_and_runs(args)
        version = build_ci_version(releases, runs, args.channel, args.current_run_id, args.current_run_number)
    except Exception as exc:
        sys.stderr.write(f"Failed to generate CI version: {exc}\n")
        raise
