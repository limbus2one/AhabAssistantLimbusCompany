...
> 基于 `main` 分支，本文档描述队伍（Team）设置数据的**定义 → 持久化 → UI编辑**完整链路。
...
- `teams_active_queue: List[int]` — 镜牢启用队伍执行队列（单一事实源）
...
| `_migrate_legacy_team_queue_from_data()` | #367 | 从旧字段 (`teams_order`/`teams_be_select`) 一次性迁移队列 |
| `_sync_team_queue()` | #398 | 将归一化后的队列写回 `teams_active_queue` |
...
4. 从 `cfg.config.teams[team_num - 1]` 获取当前槽位的 `TeamSetting`
...
| 方法 | 行号 | 说明 |
|------|------|------|
| `setting_team()` | #339 | mediator 信号槽：接收单项修改，更新 `team_setting` 并立即保存 |
| `cancel_team_setting()` | ~#380 | 返回上一页 |
...
TeamSettingCard.setting_team(data_dict)     ← mediator 信号槽单项修改
    │
    ├─ 修改 self.team_setting（即 cfg.config.teams[team_num - 1]）
    │
    └─ cfg.save() → _save_config() → config.yaml (磁盘)
```

**持久化格式：** `config.yaml` 中 `teams` 字段为 `[{...字段}, {...字段}, ...]`，每个字段值由 `TeamSetting.model_dump()` 序列化。镜牢队伍启用与执行顺序以 `teams_active_queue` 为单一事实源；旧字段 `teams_be_select` / `teams_order` 只在读取旧配置时用于一次性迁移。
